#!/usr/bin/env python3
"""
Enhanced bot.py with market cap tracking from first alert
...
- Tracks initial market cap/FDV and shows historical comparison
...
from supabase_utils import download_overlap_results, upload_file
...
POLL_INTERVAL_SECS = int(os.getenv("POLL_INTERVAL_SECS", str(60)))  # 1 minutes default
...
def parse_marketcap_value(value_str: str) -> Optional[float]:
    """Parse market cap string like '$1,234,567' or '$1.2M' into float value."""
...
def format_marketcap_display(value: Optional[float]) -> str:
    """Format a numeric market cap value for display."""
    if value is None:
        return "N/A"
...
def fetch_marketcap_and_fdv(mint: str) -> Tuple[Optional[float], Optional[float], str]:
    """
    Fetch market cap and FDV from DexScreener API.
    Returns: (marketcap_float, fdv_float, display_string)
    """
    try:
        ...
        primary_value = mc if mc else fdv
        display = format_marketcap_display(primary_value)
...
def format_alert_html(
    ...
    # Format market cap line with historical comparison
    if current_mc is not None and initial_mc is not None:
        mc_line = f"💰 <b>Market Cap:</b> {format_marketcap_display(current_mc)} (was {format_marketcap_display(initial_mc)})"
    elif current_fdv is not None and initial_fdv is not None:
    ...
    # Add first alert timestamp if available
    if first_alert_at:
        try:
            alert_date = datetime.fromisoformat(first_alert_at.rstrip("Z")).strftime("%Y-%m-%d")
            lines.append(f"🕒 <b>First alert:</b> {alert_date}")
        except Exception:
...
# ----------------------
# Background loop with market cap tracking
# ----------------------
