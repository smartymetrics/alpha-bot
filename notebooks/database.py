#!/usr/bin/env python3
"""
Full corrected bot.py
- Reads overlap_results.pkl (./data/overlap_results.pkl) produced by token_monitor.py
- Sends alerts when a token is new or grade changes
- Persists user prefs, user stats, and alerts_state and uploads them to Supabase (if configured)
"""
...
OVERLAP_FILE = DATA_DIR / "overlap_results.pkl"       # token_monitor writes this
...
POLL_INTERVAL_SECS = int(os.getenv("POLL_INTERVAL_SECS", str(30 * 60)))  # 30 minutes default
...
# ----------------------
# Thread-safe file access (joblib)
# ----------------------
file_lock = Lock()
...
def fetch_marketcap_or_fdv(mint: str) -> str:
    try:
        ...
        fdv = pairs[0].get("fdv")
        return f"${mc:,.0f}" if mc else (f"${fdv:,.0f}" if fdv else "N/A")
    except Exception as e:
...
def upload_file_to_supabase(local_path: Path, remote_filename: str) -> bool:
    """Upload a file to Supabase storage bucket (best-effort)."""
    if not SUPABASE_URL or not SUPABASE_KEY:
        logging.debug("Supabase not configured; skipping upload of %s", remote_filename)
        return False
    ...
    try:
        ...
        if resp.status_code in (200, 201):
            logging.info("Uploaded %s to Supabase as %s", local_path, remote_filename)
            return True
        else:
            logging.warning("Supabase upload failed for %s: %s %s", remote_filename, resp.status_code, resp.text)
            return False
    except Exception as e:
        logging.exception("Error uploading %s to Supabase: %s", remote_filename, e)
        return False
...
def upload_bot_data_to_supabase():
    ...
    for remote_name, local_path in files_to_upload:
        try:
            ...
            logging.debug("upload_bot_data_to_supabase: error %s", e)

def download_bot_data_from_supabase():
    """Try to download persisted bot files from Supabase public storage endpoint (best-effort)."""
    if not SUPABASE_URL or not SUPABASE_KEY:
        logging.debug("Supabase not configured; skipping downloads")
        return
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                with open(local_path, "wb") as f:
                    f.write(r.content)
                logging.info("Downloaded %s from Supabase to %s", remote_name, local_path)
        except Exception as e:
            logging.debug("Could not download %s: %s", remote_name, e)
...
def format_alert_html(token_data: Dict[str, Any], alert_type: str, previous_grade: str = None) -> str:
    ...
    mint = token_data.get("token") or token_meta.get("mint") or ""

    marketcap = fetch_marketcap_or_fdv(mint)
    overlap_pct = token_data.get("overlap_percentage") or "N/A"
    concentration = token_data.get("concentration") or "N/A"

    solscan = f"https://solscan.io/token/{mint}" if mint else ""
    gmgn = f"https://gmgn.ai/sol/token/{mint}" if mint else ""

    header = "🚀 <b>New Token Detected</b>" if alert_type == "NEW" else "🔁 <b>Grade Changed</b>"
    if alert_type == "CHANGE" and previous_grade:
        direction = "⬆️ Upgraded" if grade != previous_grade and grade in ALL_GRADES and previous_grade in ALL_GRADES and ALL_GRADES.index(grade) < ALL_GRADES.index(previous_grade) else "🔁 Grade Changed"
        header = f"{direction} — <b>{grade}</b>"

    lines = [
        header,
        f"<b>{name}</b> ({symbol})",
        f"<b>Grade:</b> {grade}" + (f" (was {previous_grade})" if previous_grade else ""),
        f"<b>Market Cap/FDV:</b> {marketcap}",
        f"<b>Overlap:</b> {overlap_pct}",
        f"<b>Concentration:</b> {concentration}",
        "",
        f'<a href="{solscan}">Solscan</a>' + (f' | <a href="{gmgn}">GMGN</a>' if gmgn else "")
    ]
    return "\n".join(lines)
...
# ----------------------
# load latest tokens from overlap_results.pkl
# ----------------------
def load_latest_tokens_from_overlap() -> Dict[str, Dict[str, Any]]:
    """Return mapping token_id -> latest_check_dict from overlap_results.pkl"""
    ...
    for token_id, history in data.items():
        if not history:
            continue
        latest = history[-1]  # most recent entry
        # ensure token id present in structure for convenience
        # overlap store uses different shapes; normalize some fields:
        norm = dict(latest)
        # some keys might be named 'token' rather than including token_metadata.mint
        if not norm.get("token_metadata"):
            # try to construct minimal token_metadata
            norm["token_metadata"] = {
                "mint": token_id,
                "symbol": norm.get("token_metadata", {}).get("symbol") if isinstance(norm.get("token_metadata"), dict) else None,
                "name": norm.get("token_metadata", {}).get("name") if isinstance(norm.get("token_metadata"), dict) else None
            }
        # make sure grade key exists
        norm["grade"] = norm.get("grade") or norm.get("classification") or "NONE"
        # expose token id on one of the keys for convenience
        norm["token"] = norm.get("token") or token_id
        latest_tokens[token_id] = norm
    return latest_tokens

# ----------------------
# Alert sending function (respect user prefs)
# ----------------------
async def send_alert_to_subscribers(app: Application, token_data: Dict[str, Any], grade: str, previous_grade: str = None):
    """
    ...
    message = format_alert_html(token_data, "CHANGE" if previous_grade else "NEW", previous_grade)
...
async def background_loop(app: Application):
    ...
    alerts_state = safe_load(ALERTS_STATE_FILE, {})  # map token_id -> last_grade_sent
    first_run = True
    ...
    while True:
        try:
            ...
            for token_id, token in tokens.items():
                ...
                last_grade = alerts_state.get(token_id)
                if grade != last_grade:
                    logging.info("New/changed grade for %s: %s -> %s", token_id, last_grade, grade)
                    await send_alert_to_subscribers(app, token, grade, previous_grade=last_grade)
                    alerts_state[token_id] = grade
