...
class FeatureExtractor:
    ...
    FEATURE_MAPPING = {
        ...
        # Smart Money Features (New)
        'sm_weighted_score': 'sm_weighted_score',
        'sm_elite_wallets': 'sm_elite_wallets',
        'sm_strong_wallets': 'sm_strong_wallets',
        'sm_active_wallets': 'sm_active_wallets',
        'sm_positive_wallets': 'sm_positive_wallets',
        'sm_negative_wallets': 'sm_negative_wallets',
        'sm_has_cluster': 'sm_has_cluster',
        'sm_cluster_size': 'sm_cluster_size',
        'sm_sniper_count': 'sm_sniper_count',
        'sm_early_buyer_count': 'sm_early_buyer_count',
        'sm_sniper_detected': 'sm_sniper_detected',
        'sm_early_buyer_detected': 'sm_early_buyer_detected',
        'sm_alpha_score': 'sm_alpha_score',
        'sm_wallet_conviction_pct': 'sm_wallet_conviction_pct',
        'sm_cluster_avg_win_rate_pct': 'sm_cluster_avg_win_rate_pct',
    ...
    def extract_features_from_snapshot(self, snapshot: dict) -> dict:
        ...
        # --- SMART MONEY DATA ---
        sm_block = features_block.get('smart_money_features', {})
        if not isinstance(sm_block, dict):
            sm_block = {}
            
        features_dict['sm_weighted_score'] = sm_block.get('sm_weighted_score', 0.0)
        features_dict['sm_elite_wallets'] = sm_block.get('sm_elite_wallets', 0)
        features_dict['sm_strong_wallets'] = sm_block.get('sm_strong_wallets', 0)
        features_dict['sm_active_wallets'] = sm_block.get('sm_active_wallets', 0)
        features_dict['sm_positive_wallets'] = sm_block.get('sm_positive_wallets', 0)
        features_dict['sm_negative_wallets'] = sm_block.get('sm_negative_wallets', 0)
        features_dict['sm_has_cluster'] = int(bool(sm_block.get('sm_has_cluster', False)))
        features_dict['sm_cluster_size'] = sm_block.get('sm_cluster_size', 0)
        features_dict['sm_sniper_count'] = sm_block.get('sm_sniper_count', 0)
        features_dict['sm_early_buyer_count'] = sm_block.get('sm_early_buyer_count', 0)
        features_dict['sm_sniper_detected'] = int(bool(sm_block.get('sm_sniper_detected', False)))
        features_dict['sm_early_buyer_detected'] = int(bool(sm_block.get('sm_early_buyer_detected', False)))
        features_dict['sm_alpha_score'] = sm_block.get('sm_alpha_score', 0)
        features_dict['sm_wallet_conviction_pct'] = sm_block.get('sm_wallet_conviction_pct', 0.0)
        features_dict['sm_cluster_avg_win_rate_pct'] = sm_block.get('sm_cluster_avg_win_rate_pct', 0.0)
