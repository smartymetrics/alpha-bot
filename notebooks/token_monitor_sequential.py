...
class TokenDiscovery:
    ...
    def fetch_dune_force_refresh(self) -> list[dict]:
        ...
        while True:
            ...
            # Check the state - use the correct attribute access
            state = getattr(status, 'state', None)
            if state is None and hasattr(status, 'status'):
                state = getattr(status, 'status', None)
            
            if self.debug:
                print(f"[Dune] execution status: {state}")
...
class DuneWinnersBuilder:
    ...
    async def build_today_from_dune(self, token_discovery: TokenDiscovery, holder_agg: 'HolderAggregator') -> Dict[str, List[str]]:
        ...
        if self.debug:
            print(f"[DuneBuilder] tokens returned by Dune: {len(starts)}")
        ...
        # Process tokens sequentially first to debug the issue
        token_to_top_holders: Dict[str, List[str]] = {}
        
        for s in starts[:5]:  # Test with first 5 tokens only
            if not s.mint:
                continue
            
            try:
                if self.debug:
                    print(f"[DuneBuilder] Processing token: {s.mint}")
                
                holders = await self._fetch_top_sampled_holders(holder_agg, s.mint)
                token_to_top_holders[s.mint] = holders
                
                if self.debug:
                    print(f"[DuneBuilder] {s.mint}: {len(holders)} final holders")
                ...
                if self.debug:
                    print(f"[DuneBuilder] Outer exception for {s.mint}: {e}")
                    import traceback
                    traceback.print_exc()

        # If sequential processing works, then process the rest concurrently
        if any(len(holders) > 0 for holders in token_to_top_holders.values()):
            if self.debug:
                print("[DuneBuilder] Sequential processing worked, processing remaining tokens")
            ...
            if tasks:
                ...
                for mint, res in zip(mints, results):
                    if isinstance(res, Exception):
                        if self.debug:
                            print(f"[DuneBuilder] failed to fetch holders for {mint}: {res}")
                        continue
                    token_to_top_holders[mint] = res
        else:
            if self.debug:
                print("[DuneBuilder] Sequential processing failed, there might be an RPC issue")

        # Save to per-day file
        total_unique_wallets = len({w for holders in token_to_top_holders.values() for w in holders})
        if self.debug:
            print(f"[DuneBuilder] Final result: {len(token_to_top_holders)} tokens with {total_unique_wallets} unique wallets")
...
class Monitor:
    ...
    async def ensure_dune_holders(self):
        """
        Ensure the 7-day Dune winners cache exists by loading last 7 days and building today's file if missing.
        """
        token_to_top_holders, wallet_freq, loaded_days = self.dune_cache.load_last_7_days()
        # If today's file not present, build it
        today_key = datetime.now(timezone.utc).strftime("%Y%m%d")
        if today_key not in loaded_days:
            if self.debug:
                print("Monitor: today's Dune cache missing, building today's cache from Dune")
            await self.dune_builder.build_today_from_dune(self.token_discovery, self.holder_agg)
        else:
            if self.debug:
                print(f"Monitor: loaded Dune cache days: {loaded_days}")
