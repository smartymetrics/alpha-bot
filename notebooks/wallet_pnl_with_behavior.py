...
Keep your API keys at the top before running.
"""
...
def compute_behavior_metrics(processed_trades: pd.DataFrame, within_seconds: int = 5, 
    ...
    Args:
        ...
        interval_days: 0 = all time (default), 7 = last 7 days, 30 = last 30 days, etc.
    """
    ...
    # Filter by time window if interval_days > 0
    if interval_days > 0:
...
def format_behavior_panel(behavior: Dict[str, Any]) -> str:
    """
    Format a compact phishing-check style panel. Requires the dict produced by compute_behavior_metrics.
    """
    if not behavior:
    ...
    partial_not_bought_count = behavior.get('sell_trades_with_not_bought_count', 0)
    # Blacklist placeholder (requires external blacklist dataset to compute). Keep 0 for now.
    blacklist_count = 0
    blacklist_pct = 0.0
    ...
    # Blacklist
    lines.append(f"🟢 Blacklist:\n  {blacklist_count} ({blacklist_pct:.0f}%)")
    # Didn't buy: sell trades fully not bought
    lines.append(f"🟢 Didn't buy:\n  {sell_not_bought_count} ({sell_not_bought_pct:.0f}%)")
    # Sold > Bought
    lines.append(f"🔴 Sold > Bought:\n  {sold_gt_bought_count} ({sold_gt_bought_pct:.0f}%)")
    # Fast sells
    lines.append(f"🟢 Buy/Sell within fast window:\n  {sells_within} ({sells_within_pct:.0f}%)")
...
def main():
    ...
    if isinstance(pnl_breakdown, dict) and 'error' not in pnl_breakdown:
        ...
        for mint, data in sorted_breakdown[:10]:  # Show top 10 tokens
            print(f"\nToken: {mint}")
        ...
        if len(sorted_breakdown) > 10:
            print(f"\n... and {len(sorted_breakdown) - 10} more tokens")
    else:
