...
class SmartMoneyScorer:
    ...
    async def score_token(
        ...
        # Cluster detection — uses real Moralis buy timestamps, not wall clock
        cluster_wallets = self._detect_smart_money_cluster(
            positive_entities, profiles, wallet_buy_timestamps
        )
    ...
    def _detect_smart_money_cluster(
        ...
        """
        Detect 3+ positive-tier wallets that bought the same token within
        cluster_window_seconds of each other, using their REAL buy timestamps
        from Moralis swap history.

        Previously used wall-clock time (time.time()) which was wrong —
        all wallets are discovered simultaneously so they all got the same
        timestamp. This version uses the actual blockTimestamp from each
        wallet's swap transaction.

        Requirements for a cluster:
          - At least CLUSTER_MIN_WALLETS (3) positive wallets
          - Each wallet must have a real buy timestamp available
          - All timestamps must fall within cluster_window_seconds (default 30 min)
          - Each wallet counts as one independent signal (address deduplication)

        Returns the largest set of wallets that form a valid cluster,
        or [] if no cluster is found.
        """
        if not positive_wallets or not wallet_buy_timestamps:
            return []

        # Build (wallet, unix_timestamp) list for positive wallets that have
        # a real buy timestamp available
        events: List[Tuple[str, float]] = []
        for wallet in positive_wallets:
            ts_iso = wallet_buy_timestamps.get(wallet)
            if not ts_iso:
                continue
            try:
                ts_dt = datetime.fromisoformat(ts_iso.replace("Z", "+00:00"))
                events.append((wallet, ts_dt.timestamp()))
            except Exception:
                continue

        if len(events) < CLUSTER_MIN_WALLETS:
            return []

        # Sort by actual buy time ascending
        events.sort(key=lambda x: x[1])

        best_cluster: List[str] = []

        # Sliding window: try every starting point, find the largest window
        # where all wallets bought within cluster_window_seconds of the first
        for i in range(len(events)):
            window: List[str] = []
            seen_addresses: Set[str] = set()
            t_start = events[i][1]

            for wallet, t in events[i:]:
                if t - t_start > self.cluster_window_seconds:
                    break
                if wallet not in seen_addresses:
                    window.append(wallet)
                    seen_addresses.add(wallet)

            if len(window) >= CLUSTER_MIN_WALLETS and len(window) > len(best_cluster):
                best_cluster = window

        if best_cluster and self.debug:
            span_secs = events[-1][1] - events[0][1] if len(events) > 1 else 0
            print(
                f"[SmartMoney] 🌊 CLUSTER detected: {len(best_cluster)} wallets "
                f"bought within {span_secs/60:.1f} min of each other"
            )
    ...
    async def _build_wallet_profiles(
        ...
        async def _profile_one(wallet: str) -> Tuple[str, WalletSmartMoneyProfile]:
            ...
            # _check_insider always returns a 2-tuple: (minutes|None, type|None)
            if isinstance(insider_result, tuple) and len(insider_result) == 2:
                insider_rank, insider_type = insider_result
            else:
                insider_rank, insider_type = None, None
